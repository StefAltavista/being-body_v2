...
export default function OilCard({ oil, reverse = false }: OilCardProps) {
  ...
  return (
    <article className="oil-card relative grid w-full grid-cols-1 items-center gap-8 overflow-hidden rounded-[2.5rem] border border-white/60 bg-white/30 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_24px_70px_rgba(62,91,112,0.13)] backdrop-blur-xl sm:p-8 lg:grid-cols-[minmax(280px,0.82fr)_minmax(0,1.18fr)] lg:gap-12 lg:p-10">
      <div
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <p className="!mt-1 !text-center !text-[17px] tracking-[0.18em]">
            {oil.intention}
          </p>
          <p className="handWrite2 !mt-5 !text-center !text-[21px] leading-relaxed">
            {oil.ingredients}
          </p>
        </div>

        <div className="relative z-10 my-7 flex h-[190px] w-[190px] items-center justify-center rounded-full border border-white/70 bg-white/30 shadow-[inset_0_5px_18px_rgba(255,255,255,0.9),0_12px_32px_rgba(72,104,126,0.12)] sm:h-[220px] sm:w-[220px]">
          <Image
            className="h-[72%] w-[72%] object-contain opacity-65"
            src="/icons/oil12.svg"
            width={220}
            height={220}
            alt={`${oil.name} oil bottle placeholder`}
          />
        </div>
      ...
      >
        <blockquote className="relative mb-9  py-3 pl-7 pr-2 sm:pl-9">
          <p className="!handWrite2 !text-center !text-[22px] leading-[1.75] sm:!text-[24px]">
        ...
        <p className="!text-left !text-[21px] leading-relaxed">
          {oil.description}
        </p>
      </div>
    </article>
