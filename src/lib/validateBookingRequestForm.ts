...
const ALLOWED_DURATIONS = ["30min", "1h", "1.5h"] as const;
...
export function validateBookingRequestForm(
  ...
  const sanitized: bookingRequestDataType = {
    ...
    sessionDuration: normalizeOneLine(input.sessionDuration!).slice(
      0,
      MAX.sessionDuration,
    ),
  };
  ...
  } else if (!/^[\p{L}\p{M} /-]{1,40}$/u.test(sanitized.pronouns)) {
  ...
  if (!sanitized.sessionDuration) {
    errors.sessionDuration = "Session duration is required.";
  } else if (
    !ALLOWED_DURATIONS.includes(
      sanitized.sessionDuration as (typeof ALLOWED_DURATIONS)[number],
    )
  ) {
    errors.sessionDuration = "Invalid session duration.";
  }
